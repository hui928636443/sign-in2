...
用于 LinuxDO 论坛日常签到和浏览帖子。不填则跳过 LinuxDO 签到。

| Secret 名称 | 必填 | 说明 |
|------------|------|------|
| `LINUXDO_ACCOUNTS` | 否 | JSON 数组格式，支持多账号 |
...
最小配置（仅浏览 LinuxDO）：
```
LINUXDO_ACCOUNTS = [{"username":"user","password":"pass"}]
```

推荐配置（LinuxDO 浏览 + NewAPI 站点签到 + 邮件通知）：
```
LINUXDO_ACCOUNTS = [{"username":"user","password":"pass"}]
...
可在 `.github/workflows/daily-check-in.yml` 中修改 cron 表达式。
